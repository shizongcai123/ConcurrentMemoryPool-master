...
Span* CentralCache::GetOneSpan(SpanList* spanlist, size_t bytes)
{
	...
	Span* newSpan = PageCache::GetInstance()->NewSpan(npage);//这个span是从pagecache中pop出来的。

	// 获取到了newSpan之后，这个newSpan是npage数量的页
	// 需要将这个span分割成一个个的bytes大小的内存块连接起来
	// 地址设置为char*是为了一会儿可以方便的挂链，每次加数字的时候，可以直接移动这么多的字节
	char* start = (char*)(newSpan->_pageid << PAGE_SHIFT);//将id转换为内存地址。

	// end是当前内存块的最后一个字节地址的下一个字节地址
	char* end = start + (newSpan->_npage << PAGE_SHIFT);//有npage页，每页4Kb，共npage<<PAGE_SHIFT byte的内存。end现在是内存块末尾地址。

	char* next = start + bytes;
	char* cur = start;
	while (next < end)//对span进行bytes大小的切片，形成一个链表形式。和freelist类似。star表示内存的开始地址，NEXT_OBJ(star)指向下一个bytes的开始地址，以此类推。end表示最后一个bytes大小内存块的开始地址。
	{
		NEXT_OBJ(cur) = next;
		cur = next;
		next = cur + bytes;
...
size_t CentralCache::FetchRangeObj(void*& start, void*& end, size_t num, size_t bytes)//num:需要获取num个bytes大小的内存块。
{
	//求出在span链表数组中的下标
