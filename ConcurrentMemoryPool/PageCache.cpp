...
Span* PageCache::NewSpan(size_t npage)
{
	...
	if (npage >= NPAGES)
	{
		void* ptr = SystemAlloc(npage);//ptr是申请到的内存的首地址
		// 构造span去管理从系统申请的内存
		Span* span = new Span();
		span->_pageid = (PageID)ptr >> PAGE_SHIFT;//用(PageID)ptr将十六进制的地址转换为10进制，由于我们向堆申请内存时都是按页进行申请的，因此我们直接将该地址除以一页的大小即可得到对应的页号。
		span->_npage = npage;
...
Span* PageCache::_NewSpan(size_t npage)
{
	...
	for (size_t i = npage + 1; i < NPAGES; i++)
	{
		if (!_pagelist[i].Empty())
		{
			...
			// 页号:从span的后面进行切割
			split->_pageid = span->_pageid + span->_npage - npage;//因为内存分配时以页为单位，所以span->_pageid向后移动span->_npage - npage距离。那么split->_pageid就表示span后npage页的id
			// 页数
			split->_npage = npage;
			...
			{
				_id_span_map[split->_pageid + i] = split;//_split->_pageid+1、_split->_pageid+2....._split->_pageid+i都在一个span里，即split.
			}

			_pagelist[span->_npage].PushFront(span);//将剩下的装入对应的span。
...
void PageCache::RelaseToPageCache(Span* span)
{
	...
	while (previt != _id_span_map.end())
	{
		...
		delete(span);//删除了span指针指向的span对象，现在span是一个悬空指针。
		span = prevspan;//让span指针指向prevspan指向的span对象。
